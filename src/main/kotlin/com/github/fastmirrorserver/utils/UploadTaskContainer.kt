...
class UploadTaskContainer {
    ...
    fun createTask(manifest: Manifest, uri: String): Task? {
        if(has(manifest)) return null
        val task = Task(
            uri = uri,
            builder = FileWriter.Builder(File(manifest.filepath)),
            expired = LocalDateTime.now().plusMinutes(10)
        )
        task_pool[manifest.key] = task
        return task
    }
